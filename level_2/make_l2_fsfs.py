...
for dir in subdirs:
  ...
  ntpts = len(featdirs)
  if ntpts == 6:
      replacements = {"OUTDIR": outdir, "NTPTS": ntpts, "FEATDIR1": '"%s"'%(featdirs[0]), "FEATDIR2": '"%s"'%(featdirs[1]), "FEATDIR3":'"%s"'%(featdirs[2]), "FEATDIR4": '"%s"'%(featdirs[3]), "FEATDIR5": '"%s"'%(featdirs[4]), "FEATDIR6": '"%s"'%(featdirs[5])}
      with open("%s/level_2/template_l2.fsf"%(server_scripts)) as infile:
  elif ntpts == 5:
      replacements = {"OUTDIR": outdir, "NTPTS": ntpts, "FEATDIR1": '"%s"'%(featdirs[0]), "FEATDIR2": '"%s"'%(featdirs[1]), "FEATDIR3":'"%s"'%(featdirs[2]), "FEATDIR4": '"%s"'%(featdirs[3]), "FEATDIR5": '"%s"'%(featdirs[4])}
      with open("%s/level_2/template_l2_r5.fsf"%(server_scripts)) as infile:
  elif ntpts == 3:
      replacements = {"OUTDIR": outdir, "NTPTS": ntpts, "FEATDIR1": '"%s"'%(featdirs[0]), "FEATDIR2": '"%s"'%(featdirs[1]), "FEATDIR3":'"%s"'%(featdirs[2])}
